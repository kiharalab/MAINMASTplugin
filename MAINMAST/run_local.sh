...
NR=/net/kihara/gterashi/DB/nr90
if [ -e /poas-scratch/gterashi/nr90 ];then
 NR=/poas-scratch/gterashi/nr90
fi
if [ -e /maroon-scratch/gterashi/nr90 ];then
 NR=/maroon-scratch/gterashi/nr90
fi
...
########
#xdir=$(dirname $0)
xdir=/net/kihara/gterashi/PredSS/SPIDER2_local/misc
if [ $# -lt 1 ]; then
	echo "usage: $0 *"
...
for seq1 in $*; do
	...
	if [ ! -f $pro1.pssm ]; then
		psiblast -db $NR -num_iterations 3 -num_alignments 1 -num_threads 4 -query $pro1.seq -out_ascii_pssm $pro1.pssm > /dev/null
